...
nhpi_data <- nhpi_data %>% mutate(CMA= ifelse(grepl(",",GEO),sub(",.*","",GEO),"N/A")) %>% filter(CMA %in% c("Vancouver","Ottawa-Gatineau","Victoria"))
...
```{r}
nhpi_data %>% filter(`New housing price indexes`=="House only") %>%
ggplot(aes(x=Date,y=VALUE,colour=CMA,linetype=CMA)) + geom_smooth()+ labs(title="New Home Prices",x=NULL,y="Housing Prices") + theme_bw()

nhpi_data %>% filter(CMA=="Edmonton") %>%
ggplot(aes(x=Date,y=VALUE,colour=`New housing price indexes`,linetype=`New housing price indexes`))+ geom_smooth()+ labs(title="New Home Prices",x=NULL,y="Housing Prices")
...
search_ct <- cmhc_cma_translation_data

rms_data_edmonton <- get_cmhc(survey = "Rms", series = "Rental Universe", dimension = "Bedroom Type", 
         breakdown = "Historical Time Periods", geo_uid = "48835")
rms_data_edmonton %>% knitr::kable()
...
```{r}
get_cmhc(survey = "Rms", series = "Rental Universe", dimension = "Bedroom Type", breakdown = "Historical Time Periods", geo_uid = "4811061") |>
  filter(`Bedroom Type` != "Total")|>
  ggplot(aes(x=Date,y=Value,fill=`Bedroom Type`)) + geom_bar(stat="identity")+
  scale_y_continuous(labels=scales::comma) +  labs(title="Edmonton rental universe",x=NULL,y="Number of units",caption="Data: CMHC RMS")
```

## Exploring CMHC Data

```{r}
under_construction <- get_cmhc(survey = "Scss", series = "Starts", dimension = "Dwelling Type", breakdown = "Historical Time Periods", geo_uid = 48835) 


under_construction %>% ggplot(aes(x=Date,y=Value,colour=`Dwelling Type`))+  geom_smooth() +
  scale_y_continuous(labels=scales::comma) +
  labs(title="City of Edmonton dwelling units under construction",
       x=NULL,y="Number of units", caption="CMHC Scss")
```

## Exploring CMHC Data 2

```{r}
select_cmhc_table()

abbotsford_chn <- get_cmhc(survey = "Core Housing Need", series = "Housing Standards", dimension = "Households in Core Housing Need", breakdown = "Historical Time Periods", geo_uid = 5909052) 

abbotsford_chn %>% ggplot(aes(x=Date,y=Value,colour=`Households in Core Housing Need`)) + geom_line()

toronto_chn <- get_cmhc(survey = "Core Housing Need", series = "Housing Standards", dimension = "Households in Core Housing Need", breakdown = "Historical Time Periods", geo_uid = 35535) 

toronto_chn %>% filter(!`Households in Core Housing Need` %in% c("Total","Above Standards")) %>% ggplot(aes(x=Date,y=Value,fill=`Households in Core Housing Need`)) + geom_bar(stat="identity")+
  scale_y_continuous(labels=scales::comma)

toronto_chn <- toronto_chn %>% mutate(CMA="Toronto")
abbotsford_chn <- abbotsford_chn %>% mutate(CMA="Abbotsford")

chn_total <- rbind(toronto_chn,abbotsford_chn)

chn_total %>% filter(!`Households in Core Housing Need` %in% c("Total","Above Standards")) %>% ggplot(aes(x=Date,y=Value,fill=`Households in Core Housing Need`)) + geom_bar(stat="identity")+
  scale_y_continuous(labels=scales::comma) + facet_wrap(~CMA,scales = "free_y")


select_cmhc_table()
tables <- cmhc::list_cmhc_tables()
```
...
::: {.column width="40%"}
![tongfen](https://raw.githubusercontent.com/mountainMath/tongfen/master/images/tongfen-sticker.png)
:::
:::::
