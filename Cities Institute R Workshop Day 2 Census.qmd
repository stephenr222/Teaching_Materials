...
```{r}
set_cancensus_cache_path("~/Downloads",install=TRUE,overwrite = TRUE)
...
```{r}
find_census_vectors("Australian ethnic", dataset = "CA21", type = "total", query_type = "keyword", interactive = TRUE)
```
...
ggplot(poverty_data,aes(fill=lico_at+lico_at_babies)) +
  geom_sf(size=NA) +  
